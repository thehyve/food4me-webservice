...
class ImportServiceIntegrationSpec extends IntegrationSpec {
	...
	void "test importing English translations"() {
		given: "a list of English advices to import, in the expected format"
			// Format: code, text
			def advicesToImport = [
				[ "L0.0.1", "English" ],
				[ "L0.2.5", "American" ],
		...
		when: "importing the texts in English"
			importService.loadAdviceTexts(is, "en")
		...
		and: "the default text for the advice is stored in the Advice objects as well"
			Advice.findByCode( "L0.0.1" )?.text == "English"
			Advice.findByCode( "L0.2.5" )?.text == "American"
	}
